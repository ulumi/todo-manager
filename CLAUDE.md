...
```js
{
  ...
  durationHistory?: [{ date, minutes }],  // historique des durées réelles écrites par focusComplete() ou app.logTaskDuration() (borné à 30 entrées) — alimente "Temps passé" en vue Analyse
...
- **Invite « combien de temps ça a pris ? »:** à la complétion d'une tâche (checkbox `app.toggleTodo()` en vue jour/semaine/mois, ou `app.completeWithSubtasks()`) qui n'a **jamais** eu ni estimation ni temps loggé (`app._hasNoTimeInfo(t)` : ni `durationEstimated`, ni `durationReal`, ni `durationHistory`), popover non bloquant ancré sur l'item (`app._showTimeLogPrompt()`, `.time-log-popover` — même ancrage que `.subtask-warning-popover`) : la tâche est déjà complétée à ce stade, répondre est optionnel (fermeture sur clic dehors ou ×, sans annuler la complétion). Une réponse écrit `durationReal` + append `durationHistory` via `app.logTaskDuration()`, même schéma que `focusComplete()`. Ne se déclenche que sur une complétion **individuelle** (pas `completeMany`/actions de lot, ni les zones de dépôt du Bilan) et jamais en Focus (qui a déjà son propre chrono)
